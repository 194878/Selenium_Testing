...
namespace Casekaro.Utilities
{
    internal class ExcelUtils
    {
        ...
        public static List<RegisterExcelData> ReadRegisterExcelData(string excelFilePath, string sheetName)
        {
            List<RegisterExcelData> excelDataList = new List<RegisterExcelData>();
            Encoding.RegisterProvider(CodePagesEncodingProvider.Instance);
            ...
            using (var stream = new FileStream(excelFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
            {
                using (var reader = ExcelReaderFactory.CreateReader(stream))
                {
                    ...
                    if (dataTable != null)
                    {
                        foreach (DataRow row in dataTable.Rows)
                        {
                            RegisterExcelData excelData = new RegisterExcelData
                            { 
